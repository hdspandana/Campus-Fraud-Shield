...
# Used by BOTH train/train_model.py (manual retraining) AND
# core/pipeline.py's automatic inline-training fallback (which fires
# whenever no saved model exists yet — e.g. on a fresh deploy like
# Streamlit Cloud, where nobody runs train_model.py by hand). Having
# one shared function means the metrics dashboard works correctly
# regardless of which code path actually did the training, instead of
# only being generated by the manual script.
def write_model_metrics(clf: "SemanticScamClassifier", df, metrics_path: str = "data/model_metrics.json") -> dict:
    """
    Computes honest, out-of-fold confusion matrix + precision/recall/F1
    (via cross-validation — never scoring the model on its own training
    data) plus per-category example counts, and writes them to
    metrics_path as JSON. Returns the metrics dict too.
    """
    import json as _json

    labels = clf.training_labels
    n_scam = sum(labels)
